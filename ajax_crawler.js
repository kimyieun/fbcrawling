...
function getSenderName(list) {
    ...
    if (list['Sender'] == 'User') {
        ...
        res = res.replace('오늘은 ', '').replace('과거의 오늘 ', '').split('님과');
        if (typeof (res) == "object" && res[0].search('님이|,') != -1) {
            re = /(.*?)(님이|,)/;
            res = re.exec(res[0])[1];
        }
    }
...
function getLikes(list, li, IsGroupActivity) { //(IsGroupActivity = true) -> 게시물이 하나만 뜨는것이 아니라서 다르게 접근해야함
    var content = $(li).find('a').eq(0);
    var addr = content.attr('href');
    $.ajax(addr)
        .done(function (text) {
...
function getContentInfo(list, li, IsGroupActivity) {
    ...
    if (title.search("앨범") == -1) list = getLikes(list, li, IsGroupActivity); //(Share, Comment, Likes)
    return list;
};
    ...
    try {
        ...
        if (noti_title.search('그룹') != -1) { //Group
            ...
            if ((relationship.search("page") != -1 && relationship.search("subscription") == -1) || relationship.search("event_calendar_create") != -1) { //Page
                ...
                if (noti_title.search('님이') != -1 || noti_title.search('님과') != -1) { //User
                    notification['Sender'] = 'User';
                    notification = getSenderName(notification);
                    notification = classifyUserActivities(notification, li, false);
                }
...
function downloadObjectAsJson(exportObj, exportName){
  ...
  downloadObjectAsJson(notifications, "notifications");
